...
$factory->define(App\Asset::class, function (Faker\Generator $faker) {
    ...
    $data = [
        ...
        'user_id' => $faker->unique(true)->numberBetween(1,11),
        'label' => $faker->unique(true)->word(),
        'manufacturer' => $faker->unique(true)->word(),
        'model' => $faker->unique(true)->regexify('[0-9]{5}-[a-z]{2}-[0-9]{6}'),
        ...
        'cost' => $faker->numberBetween(50,100000) . "." . $faker->randomDigit() . $faker->randomDigit()  ,
        'serial_number' => $faker->unique(true)->regexify('[0-9]{2}-[0-9]{5}-[0-9]{6}'),
        'po_number' => $faker->regexify('po-[0-9]{9}'),
        'checkoutable' => $faker->boolean(),
        'category' => $faker->randomElement($array = array ('','computer', 'laptop', 'tablet', 'projector', 'document camera', 'extron', 'camera', 'lens', 'printer', 'audio', 'display', 'peripheral', 'equipment')),
        'lifecycle' => $faker->numberBetween(0, 10),
...
$factory->define(App\Campus::class, function (Faker\Generator $faker) {
    ...
    $data = [
        'id' => null,
        'name' =>$faker->word(),
        'code' => $faker->word() . "_" . $faker->randomDigit(),
    ];
    // dd($data);
    return $data;
});
...
$factory->define(App\Campus::class, function (Faker\Generator $faker) {
    ...
    $data = [
        'id'=> null,
        'name' =>  $faker->unique()->word(),
        'code' => $faker->unique()->regexify('\w{3}_\d{3}')
    ];
    // dd($data);
    return $data;
});
