<template>
  <div>
    <h2>Users</h2>
    <div id="hidden-actions" class="container">
        ...
        <div class="well">
          <form id="addUserForm">
            <div class="form-group">
              ...
              <input type="text" class="form-control" v-model="user.first_name" placeholder="first_name" required>
            </div>
            <div class="form-group">
              <label for="last_name">Last name</label>
              <input type="text" class="form-control" v-model="user.last_name" placeholder="last_name" required>
            </div>
            <div class="form-group">
            ...
            <div>
              <button type="submit" @click="submitUser($event)"> <i class="fas fa-plus-square"></i> Add User</button>
            </div>
          </form>
        </div>
      </div>
    ...
    <div class="btn-group">
      <button><i class="fas fa-user-plus" data-toggle="collapse" href="#addUserContainer"></i></button>
    </div>
      ...
      <tbody>
        ...
        <template v-for="(user, userKey) in results">
          <tr @click="select(user)" >
            <td data-toggle="collapse" :data-target="'#row'+userKey" class="accordion-toggle">
    ...
    methods: {
      submitUser: function(e){
        ...
        .then(function (response) {
          ...
          vm.results.unshift(user)
          console.log(user);
          toastr.success("Added User: " + user.first_name + " " + user.last_name)
        })
        .catch(error => {
      ...
      onSelectDepartment: function(e){
        ...
        axios.post('/api/v1/users/'+ this.currentUser.id +'/sync/roles', this.currentUser.roles)
        .then(function (response) {
         // console.clear();
         var items = response.data
         console.log(items);
