...
class UserTest extends TestCase
{
    ...
    public function it_uses_request_all_params(){
      //Arrange
      $users = $this->createUsersWithRoles();
      
      //Act
      $response = $this->actingAs($users['admin'], 'api')->get('/api/v1/users?search=demo&searchColumns=username&with=roles,departments&fields=email,id,username');
    ...
    public function it_can_request_with(){
        ...
        //Act
        $response = $this->actingAs($users['admin'], 'api')->get('/api/v1/users?with=roles,departments');
     ...
     public function it_can_request_search(){
        $users = $this->createUsersWithRoles();
        $response = $this->actingAs($users['admin'],'api')->get('/api/v1/users?search=de');
        $this->assertTrue($response->status() == 200);
    }
    ...
    public function it_can_request_fields(){
        ...
        //Act
        $response = $this->actingAs($users['admin'], 'api')->get('/api/v1/users?fields=id,email');
    ...
    public function it_grabs_the_first_user(){
        ...
        //Act
        $response = $this->actingAs($users['admin'], 'api')->get('/api/v1/users/1');
    ...
    public function it_grabs_the_first_user_with_withs(){
        ...
        //Act
        $response = $this->actingAs($users['admin'], 'api')->get('/api/v1/users/1?with=roles,departments');
    ...
    public function it_grabs_the_first_user_with_fields(){
        ...
        //Act
        $response = $this->actingAs($users['admin'], 'api')->get('/api/v1/users/1?fields=id,email');
    ...
    private function createUsersWithRoles(){
        ...
        return ['demo' => $demoUser, 'admin' => $adminUser];
    }
